document.addEventListener('DOMContentLoaded', function(){
    ...
    let setUserDetails = function(){
        ...
        linkedinLink.setAttribute('href', `https://linkedin.com/${parsedUserDetails[3]}`)
        mediumLink.setAttribute('href', `https://medium.com/@${parsedUserDetails[4]}`)
    }
    ...
    let getRandomQuote = function(){
        ...
        fetch(link).then((response) =>{ return response.json()}).then(data => {
            document.getElementById("quote_block_quote").innerHTML = `${data.content}`
            document.getElementById("quote_block_author").innerHTML = `${data.author}`
